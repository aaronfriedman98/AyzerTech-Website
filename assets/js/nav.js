...
function myFunction(x) {
    if (x.matches) {
        window.addEventListener('scroll', () => {
        if (window.scrollY >= 514 && window.scrollY <= 1510 || window.scrollY >= 2054 && window.scrollY <= 2634 || window.scrollY >= 3240 && window.scrollY <= 5145 || window.scrollY >= 5398) {
            scroll.classList.add('scrolled')
            menuName.classList.add('scrolled')
        }
        else {
